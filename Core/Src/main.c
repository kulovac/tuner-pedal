...
#define ADC_SAMPLE_TIME (2.5f / 80e6) // in s
//#define SAMPLE_RATE (1.0f / (1.0f / 2000.0f + ADC_SAMPLE_TIME))   // in Hz
...
float32_t firArr[FIR_LEN] = { -0.0026012824532543164, -0.0033000391283683167, -0.004010081650667714, -0.0019592148837829122, 0.006662701906228264, 0.024831690243449004,
		0.052737841736489564, 0.0867532118751942, 0.11997490001564744, 0.1442961800416668, 0.153228184594796, 0.1442961800416668,
		0.11997490001564744, 0.0867532118751942, 0.052737841736489564, 0.024831690243449004, 0.006662701906228264, -0.0019592148837829122,
...
float32_t get_rfft_freq(uint16_t *arr) {
	...
	for (size_t i = 1; i < FFT_BUFFER_SIZE / 2; ++i) {
		float32_t cur_val = fftBufOut[2*i]*fftBufOut[2*i] + fftBufOut[2*i+1]*fftBufOut[2*i+1];
...
void create_zoom_arr(float32_t *outArr, uint16_t *inArr, float32_t f_0) {
	...
	for (size_t i = 0; i < FFT_BUFFER_SIZE; ++i) {
		input[i] = ((float32_t)inArr[i] - 2048) * arm_cos_f32((float32_t)(2*PI*f_0 * i / SAMPLE_RATE));
	}
	...
	memset(&(dest[FFT_BUFFER_SIZE / DECIMATION * 4]), 0, (FFT_BUFFER_SIZE - FFT_BUFFER_SIZE / DECIMATION * 4) * sizeof(float32_t));
	...
	// Shift in frequency domain
	for (size_t i = 0; i < FFT_BUFFER_SIZE; ++i) {
		input[i] = -((float32_t)inArr[i] - 2048) * arm_sin_f32((float32_t)(2*PI*f_0 * i / SAMPLE_RATE));
	}
...
float32_t get_zoom_freq(float32_t *zoomArr, float32_t f_0) {
	...
	for (size_t i = 0; i < FFT_BUFFER_SIZE; ++i) {
		float32_t cur_val = zoomArr[2*i]*zoomArr[2*i] + zoomArr[2*i+1]*zoomArr[2*i+1];
		if (cur_val > largest_val) {
			largest_index = i;
			largest_val = cur_val;
		}
	...
	if (largest_index <= FFT_BUFFER_SIZE / 2) {
		zoom_freq = (float32_t)largest_index * SAMPLE_RATE / FFT_BUFFER_SIZE / DECIMATION + f_0;
	} else {
		zoom_freq = ((float32_t)largest_index - FFT_BUFFER_SIZE) * SAMPLE_RATE / FFT_BUFFER_SIZE / DECIMATION + f_0;
	}

	return 0.987880793f * zoom_freq - 0.00077004f; // Little term adjustment using LINEST correction
}
/* USER CODE END 0 */
